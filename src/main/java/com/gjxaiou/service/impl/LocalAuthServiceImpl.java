...
public class LocalAuthServiceImpl implements LocalAuthService {
    ...
    public LocalAuth getLocalAuthByUsernameAndPwd(String username, String password) {
        // 因为保存到数据库中的密码是加密之后的，所以如果要比较的话就需要先将密码进行加密然后在和数据库中比较。
    ...
    public LocalAuthExecution modifyLocalAuth(Long userId, String username, String password,
        ...
        // 非空判断，判断传入的用户 ID，账号，新旧密码是否为空，新旧密码是否相同，如果不满足条件则返回错误信息
        if (userId != null || StringUtils.isNotBlank(username) && StringUtils.isNotBlank(password)
                && StringUtils.isNotBlank(newPassword) && !password.equals(newPassword)) {
            try {
